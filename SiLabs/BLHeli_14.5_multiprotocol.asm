...
RCP_PPM_MULTISHOT			EQU 	7		;	 RC pulse ppm type input is Multishot
...
pca_int:	; Used for RC pulse timing
	...
	jnb	Flags2.RCP_PPM_MULTISHOT, pca_int_fall_not_multishot
...
pca_int_fall_not_multishot:
;	OneShot42 section commented out for now.
;	jnb	Flags2.RCP_PPM_ONESHOT42, pca_int_fall_not_oneshot_42
;
; Oneshot42 - Add 2/256
;	clr	C
;	mov	A, Temp1
...
pca_int_fall:
	...
	jnb	Flags2.RCP_PPM_MULTISHOT, ($+5)	; Is RC input Multishot?
	ajmp	pca_int_fall_oneshot				; Yes - branch forward
	
	jb	Flags2.RCP_PPM_ONESHOT125, ($+5)
	ajmp	pca_int_fall_not_oneshot

pca_int_fall_oneshot:
...
calc_governor_prop_correction:
	...
	subb	A, #0FFh
	jnc ($+5)	
	ljmp governor_limit_prop_corr_neg	; Yes - limit
	ljmp	governor_apply_prop_corr
...
; Test whether signal is OneShot42 (Currently no Oneshot42 flag, so commented out for now)
;	clr	Flags2.RCP_PPM_ONESHOT42			; Clear OneShot42 flag
;	mov	Rcp_Outside_Range_Cnt, #0		; Reset out of range counter
;	call wait100ms						; Wait for new RC pulse
;	clr	C
;	mov	A, Rcp_Outside_Range_Cnt			; Check how many pulses were outside normal range ("745-2235us")
;	subb	A, #10
;	jc	validate_rcp_start
;	setb	Flags2.RCP_PPM_ONESHOT42			; Set OneShot42 flag

; Test whether signal is Multishot
	clr	Flags2.RCP_PPM_MULTISHOT			; Clear Multishot flag
	mov	Rcp_Outside_Range_Cnt, #0		; Reset out of range counter
	call wait100ms						; Wait for new RC pulse
	clr	C
	mov	A, Rcp_Outside_Range_Cnt			; Check how many pulses were outside normal range ("745-2235us")
	subb	A, #10
	jc	validate_rcp_start
	setb	Flags2.RCP_PPM_MULTISHOT			; Set Multishot flag
